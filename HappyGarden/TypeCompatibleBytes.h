 ...
 */
template<int32 Size, uint32 Aligment>
struct TAlignedBytes; //  this intentionally will not coplile, we do not support this kind of alligment.
...
// size is a multiple of the desired alignment
	...
	#define IMPLEMENT_ALIGNED_STORAGE(Align) \
		...
		{ \
			uint8 Pad[Size];\
			static_assert(Size % Align == 0, "CLT interop types must not be aligned."); \
		};
#else
